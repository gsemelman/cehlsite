...
$CurrentHTML = '';
$CurrentTitle = $StandingsTreeTitle;
$CurrentPage = 'StandingsTree';
include 'head.php';
...
<td style="width:25%"><a style="display:block; width:100%; text-align:center;" class="lien-blanc" href="Schedule.php?plf=1&rnd=1">Round 1</a></td>
<td style="width:25%"><a style="display:block; width:100%; text-align:center;" class="lien-blanc" href="Schedule.php?plf=1&rnd=2">Round 2</a></td>
<td style="width:25%"><a style="display:block; width:100%; text-align:center;" class="lien-blanc" href="Schedule.php?plf=1&rnd=3">Round 3</a></td>
<td style="width:25%"><a style="display:block; width:100%; text-align:center;" class="lien-blanc" href="Schedule.php?plf=1&rnd=4">Round 4</a></td>
</tr><tr>
...
for($j=1;$j<=$existRnd;$j++) {
	...
	if(file_exists($Fnm)) {
		for($x=0;$x<$TeamNumber;$x++) {
			...
			if($TeamNumber != 0) {
				echo '<table class="tableau" style="border:1px solid '.$couleur_contour.'; width:190px;"><tr class="tableau-top">';
				echo '<td></td>';
				for($w=1;$w<=$m;$w++) {
					if(isset($day[$w]) && isset($score1[$w])) echo '<td style="width:8px; padding-left:2px; padding-right:2px;"><a class="lien-blanc" href="games.php?num='.$day[$w].'&rnd='.$j.'" style="display:block; width:100%;">'.$w.'</a></td>';
					else echo '<td style="width:8px; padding-left:2px; padding-right:2px;">'.$w.'</td>';
				}
				echo '<td style="width:8px; padding-left:2px; padding-right:2px;"></td>';
				echo '</tr>';
				$bold = '';
				...
				for($w=1;$w<=$m;$w++) {
					...
					echo '<td style="width:8px; padding-left:2px; padding-right:2px;'.$bold.'">'.$score2[$w].'</td>';
				}
				...
				for($w=1;$w<=$m;$w++) {
					...
					echo '<td style="width:8px; padding-left:2px; padding-right:2px;'.$bold.'">'.$score1[$w].'</td>';
				}
